...
This fork also includes a local Codex marketplace entry for the promoted skills.
...
The Codex package lives in [`plugins/matt-pocock-skills`](./plugins/matt-pocock-skills). It mirrors
the skills published by the upstream [`.claude-plugin/plugin.json`](./.claude-plugin/plugin.json),
flattens them into the layout Codex expects, and translates explicit-only invocation metadata into
Codex policy files. Rebuild it after an upstream refresh with:

```bash
node scripts/build-codex-plugin.mjs <plugin-version>
```
